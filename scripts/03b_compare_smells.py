...
if __name__ == "__main__":
    # Assume script is run after 01, 02, 03
    if not os.path.exists(METRICS_DIR):
        print(f"Error: '{METRICS_DIR}' directory not found.", file=sys.stderr)
        sys.exit(1)
        
    repo_name = None
    # Find the first subdirectory in metrics/ (assuming single repo for now)
    for item in os.listdir(METRICS_DIR):
        if os.path.isdir(os.path.join(METRICS_DIR, item)):
            repo_name = item
            break
            
    if not repo_name:
        print(f"Error: No repository metrics directory found in '{METRICS_DIR}'.", file=sys.stderr)
        print(f"Please run previous steps first.", file=sys.stderr)
        sys.exit(1)
        
    print(f"--- Comparing Smells for Repository: {repo_name} ---")
    ...
    if pylint_data is None or radon_data is None or ai_data is None:
        print("Error: Failed to load one or more necessary metric files. Aborting.", file=sys.stderr)
        sys.exit(1)
    ...
    ai_smells, num_ai_parsed = extract_ai_smells(ai_data) # Now returns PARSED count
    # Get the total originally reported by AI from its summary
    ...
    summary = {
        ...
        "ai_false_positives": fp_ai # AI smells not matching Pylint OR Radon
    }
