...
if __name__ == "__main__":
    ...
    if not os.path.exists(ORIGINAL_CODE_DIR) or not os.listdir(ORIGINAL_CODE_DIR):
        log.error(f"'{ORIGINAL_CODE_DIR}' directory is empty or does not exist. Run 01_fetch_repos.py first.")
        sys.exit(1)
    ...
    if not repo_name:
        log.error(f"No repository directory found in '{ORIGINAL_CODE_DIR}'.")
        sys.exit(1)
        
    if not generate_missing_tests(repo_name):
        log.error(f"Test generation process failed for repository: {repo_name}")
        sys.exit(1)
    else:
        log.info(f"Test generation process completed for repository: {repo_name}")
