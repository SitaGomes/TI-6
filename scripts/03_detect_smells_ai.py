...
from prompts import STANDARD_SMELL_PROMPT_TEMPLATE
...
def analyze_file_with_ai(file_path: str, client, repo_name: str):
    """Reads a file, calls the AI for smell detection, and parses the result."""
    ...
    prompt = STANDARD_SMELL_PROMPT_TEMPLATE.format(
...
def detect_ai_smells(repo_name: str):
    """Detects smells in a repository using the AI model."""
...
if __name__ == "__main__":
    ...
    # Ensure local analysis ran first (optional check, assumes user runs in order)
    # ... Add check if needed ...

    processed_repo_count = 0
    failed_repo_count = 0
    
    # Use the first directory found as the target repo (since N=1 for now)
    repo_name = None
    ...
    if not repo_name:
        print(f"Error: No repository directory found in '{ORIGINAL_CODE_DIR}'.", file=sys.stderr)
        sys.exit(1)
        
    if detect_ai_smells(repo_name):
        processed_repo_count += 1
    else:
        failed_repo_count += 1

    print("\n--- AI Smell Detection Summary ---")
